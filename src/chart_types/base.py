...
from settings import CURRENCY_SYMBOLS, MARKET_MAPPINGS
...
def calculate_currency_metrics(df, broker, currency):
    ...
    broker_currency_data = df[
        (df["broker_name"] == broker) & (df["currency"] == currency)
    ].copy()
    ...
    charges_mask = broker_currency_data["Action"].str.contains(
        "Funding charge", case=False, na=False
    )
    total_charges = broker_currency_data[charges_mask]["P/L"].sum()

    # Calculate deposits and withdrawals
    deposits = broker_currency_data[
        broker_currency_data["Action"] == "Fund rece received"
    ]["P/L"].sum()
    withdrawals = broker_currency_data[
        broker_currency_data["Action"] == "Fund payable"
    ]["P/L"].sum()
...
def convert_to_base_currency(
    amount, from_currency, exchange_rates, base_currency="SEK"
):
    """
    ...
    if from_currency in exchange_rates:
        ...
        logger.warning(
            f"Exchange rate not found for {from_currency}, using original amount"
        )
        return amount


def format_currency_with_conversion(
    value, currency, exchange_rates, base_currency="SEK", show_converted=True
):
    """
    ...
    if show_converted and currency != base_currency:
        converted_value = convert_to_base_currency(
            value, currency, exchange_rates, base_currency
        )
...
def find_date_col(df):
    """Find the date column in a DataFrame using common name variations."""
    return find_column(
        df, {"transactiondate", "transaction_date", "transaction-date", "date"}
    )


def find_pl_col(df):
    """Find the P/L (profit/loss) column in a DataFrame using common name variations."""
    return find_column(df, {"p/l", "pl", "plamount", "profitloss", "profit"})


def coerce_date(df, col):
    """Convert a column to datetime, modifying the DataFrame in place."""
...
def coerce_pl_numeric(df, col, alias="_pl_numeric"):
    """Convert P/L column to numeric, creating an alias column."""
    if col and col in df.columns:
...
def ensure_market_column(
    df: Optional[pd.DataFrame], column_name: str = "Market"
) -> pd.DataFrame:
    """
    ...
    if not isinstance(df, pd.DataFrame):
        ...
        try:
            ...
            logger.error(
                "ensure_market_column: unable to coerce input to DataFrame, returning empty DataFrame"
            )
            return pd.DataFrame()
    ...
    if desc_col:
        try:
            dfc[column_name] = (
                dfc[desc_col].astype(str).apply(_infer_market_from_description)
            )
            return dfc
        except Exception:
            logger.exception(
                "ensure_market_column: failed to infer Market from Description; falling back to Unknown"
            )
...
def normalize_trading_df(df: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    try:
        ...
        logger.exception(
            "normalize_trading_df: ensure_market_column failed; adding fallback Market='Unknown'"
        )
    ...
    if date_col:
        ...
        dfc["Transaction Date"] = pd.to_datetime(
            dfc.get("Transaction Date"), errors="coerce"
        )
    ...
    try:
        ...
        try:
            dfc["_pl_numeric"] = pd.to_numeric(
                dfc.get(pl_col, pd.Series([0] * len(dfc))), errors="coerce"
            ).fillna(0.0)
        except Exception:
            dfc["_pl_numeric"] = 0.0
    ...
    try:
        dfc["_pl_numeric"] = (
            pd.to_numeric(dfc["_pl_numeric"], errors="coerce").fillna(0.0).astype(float)
        )
    except Exception:
        logger.debug(
            "normalize_trading_df: failed to force _pl_numeric to float; leaving as-is"
        )
...
def aggregate_pl_by_period(
    ...
    agg = (
        ...
        .sum()
        .reset_index()
        .rename(columns={date_col: "Period", pl_col: pl_col})
    )
    return agg


def top_markets_by_pl(
    df: pd.DataFrame,
    pl_col: str = "_pl_numeric",
    label_col: str = "Market",
    top_n: int = 10,
) -> pd.DataFrame:
    """
...
def pick_label_col(df: pd.DataFrame, candidates=None) -> Optional[str]:
    ...
    return (
        ...
        else (df2.columns[0] if len(df2.columns) else None)
    )


def to_unified_currency(
    df: pd.DataFrame,
    exchange_rates: dict,
    base_currency: str,
    pl_col: str = "_pl_numeric",
    ...
    out = df.copy()
    out["original_currency"] = out.get(
        "Currency", out.get("currency", pd.Series([None] * len(out)))
    )
    ...
    out["_pl_in_base"] = (
        pd.to_numeric(out.get(pl_col, out.get("P/L", 0)), errors="coerce").fillna(0.0)
        * out["_conv"]
    )
    return out
