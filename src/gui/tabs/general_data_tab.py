...
class DataOverviewTab(QWidget):
    ...
    def create_data_display(self, parent_layout):
        ...
        # Summary stats table (Metric / Value) - replaces multiple single-line labels
    ...
    def _populate_stats_table(self, stats: dict):
        """Flatten stats dict and show in the 2-column stats_table (Metric / Value)."""
        try:
            if not isinstance(stats, dict):
                self.stats_table.setRowCount(0)
                return
            # Flatten values similar to points.create_points_view table formatting
            keys = []
            vals = []
                ...
                if isinstance(v, dict):
                    ...
                    for mk, mv in v.items():
                        if isinstance(mv, float):
                            formatted_items.append(f"{mk}:{mv:.1f}")
                        else:
                            formatted_items.append(f"{mk}:{mv}")
                    vals.append("; ".join(formatted_items))
                else:
                    keys.append(k)
                    vals.append(str(v))
            self.stats_table.setRowCount(len(keys))
            for i, (k, v) in enumerate(zip(keys, vals)):
                self.stats_table.setItem(i, 0, QTableWidgetItem(str(k)))
                self.stats_table.setItem(i, 1, QTableWidgetItem(str(v)))
        except Exception:
            logger.exception("Failed to populate stats table")
    ...
    def update_data(self):
        ...
        try:
            ...
            try:
                ...
                logger.debug("DataOverviewTab.update_data: raw df shape=%s columns=%s",
                             getattr(df_preview, 'shape', None),
                             list(getattr(df_preview, 'columns', [])))
            except Exception:
            ...
            df = self.data_manager.get_data()
            logger.debug("DataOverviewTab.update_data: selected year=%s, df rows=%s",
                         year_text, len(df) if df is not None else 'None')
            if df is None or df.empty:
                # clear stats table and other tables
            ...
            stats = {
                ...
                "Total Balance": f"{total_balance:.2f} {base_currency}" if base_currency else f"{total_balance:.2f}",
                "Avg P/L per Month": f"{avg_pl_per_month:.2f}",
                "Avg P/L per Day": f"{avg_pl_per_day:.2f}",
                "Avg Points per Month": f"{avg_pts_per_month:.1f}",
                "Avg Points per Day": f"{avg_pts_per_day:.1f}",
            }
